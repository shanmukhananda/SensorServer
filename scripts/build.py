...
def android_deploy(build_dir, install_dir):
    android_deployer = os.path.join(os.environ["QTKITS"], "android_armv7", "bin", "androiddeployqt.exe")
    android_deployer = os.path.normpath(android_deployer)
    deployment_settings = os.path.join(build_dir, "android-libSensorServer.so-deployment-settings.json")
...
def build_android_impl(buildtype):
    ...
    ndk_make = os.path.join(os.environ["ANDROID_NDK_HOME"], "prebuilt", "windows-x86_64", "bin", "make.exe")
    ...
    qmake_path= os.path.join(os.environ["QTKITS"], "android_armv7", "bin", "qmake.exe")
...
def main(argv):
    ...
    ap.add_argument("--target", required=True,
        help="Target platforms #:android #:windows")
    args = ap.parse_args()
    build(args)
