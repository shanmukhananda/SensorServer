...
def build_flatbuffer_android(buildtype):
    ...
    build_dir = os.path.join(project_dir, "vendor", "buildtrees", "flatbuffers", output_dir)

    utils.mkdir_p(install_dir)
    utils.mkdir_p(build_dir)

    android_toolchain = os.path.join(os.environ["ANDROID_NDK_HOME"], "build", "cmake", "android.toolchain.cmake")
    android_make = os.path.join(os.environ["ANDROID_NDK_HOME"], "prebuilt", "windows-x86_64", "bin", "make.exe")
